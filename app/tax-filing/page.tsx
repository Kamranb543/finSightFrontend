  ...
  return (
    <ProtectedRoute>
      <AppLayout title="Tax Filing Assistant">
        <div className="relative -mx-8 -mt-8 -mb-8 h-[calc(100vh-8rem)] w-[calc(100%+4rem)]">
          ...
          <div className="bg-card/50 backdrop-blur-sm border-b border-border/50 p-4 flex-shrink-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="font-semibold text-foreground">Tax Filing Assistant</h2>
                  <p className="text-sm text-muted-foreground">Ask me anything about taxes</p>
                </div>
              </div>
          ...
          {/* Messages Container */}
          <div className="flex-1 overflow-y-auto bg-card/30 backdrop-blur-sm p-6 space-y-4 scrollbar-thin">
            {messages.length === 0 && (
            ...
            {messages.map((message, index) => (
              ...
              >
                {message.sender === "bot" && (
                  <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0 animate-in zoom-in duration-300">
                ...
                {message.sender === "user" && (
                  <div className="h-8 w-8 rounded-full bg-muted flex items-center justify-center flex-shrink-0 animate-in zoom-in duration-300">
            ...
            {isTyping && (
              <div className="flex gap-3 justify-start animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                <div className="bg-muted rounded-2xl rounded-tl-sm px-4 py-3 shadow-sm animate-in zoom-in-95 duration-300">
                  <div className="flex gap-1.5 items-center">
                    ...
                    <span className="h-2 w-2 bg-muted-foreground/60 rounded-full animate-bounce [animation-delay:200ms] [animation-duration:1.4s]" />
                    <span className="h-2 w-2 bg-muted-foreground/60 rounded-full animate-bounce [animation-delay:400ms] [animation-duration:1.4s]" />
                    <span className="ml-2 text-xs text-muted-foreground/70 animate-pulse">Thinking...</span>
                  </div>
                </div>
              </div>
            )}
          ...
          {isComplete && taxSummary && (
            <div className="p-6">
              <Card className="bg-card/80 backdrop-blur-sm border-border/50">
                <CardHeader>
                  <CardTitle className="text-xl">Tax Summary for {taxSummary.tax_year}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm text-muted-foreground">Total Income</p>
                      <p className="text-lg font-semibold">Rs {taxSummary.total_income.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">Total Deductions</p>
                      <p className="text-lg font-semibold">Rs {taxSummary.total_deductions.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">Taxable Income</p>
                      <p className="text-lg font-semibold">Rs {taxSummary.taxable_income.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">Estimated Tax</p>
                      <p className="text-lg font-semibold">Rs {taxSummary.estimated_tax.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">Withholding Tax</p>
                      <p className="text-lg font-semibold">Rs {taxSummary.withholding_tax.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">
                        {taxSummary.is_refund ? "Expected Refund" : "Net Tax Payable"}
                      </p>
                      <p className={`text-lg font-semibold ${taxSummary.is_refund ? "text-green-500" : "text-destructive"}`}>
          ...
          {/* Input Form */}
          <form onSubmit={handleSend} className="bg-card/50 backdrop-blur-sm border-t border-border/50 p-4 flex-shrink-0">
